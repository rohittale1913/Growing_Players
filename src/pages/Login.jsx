...
import { Mail, Lock, AlertCircle, Loader2, ShieldAlert, CheckCircle2 } from 'lucide-react'
...
const Login = () => {
  ...
  useEffect(() => {
    const handleEmailVerification = async () => {
      try {
        // Check if user was just redirected from email confirmation
        const { data: { session }, error: sessionError } = await supabase.auth.getSession()
        
        if (session && session.user) {
          // User has been verified
          console.log('✅ Email verified! User session created.')
          ...
          >
            ...
            {verificationSuccess && (
              <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                ...
                <p className="text-green-700 text-sm font-medium">✓ Email verified successfully! Redirecting...</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 mt-6 disabled:opacity-50 ${
                  isAdminLogin ? 'btn-primary bg-red-600' : 'btn-primary'
                }`}
              >
                {(loading || isVerifying) && <Loader2 className="w-4 h-4 animate-spin" />}
                {loading ? 'Signing in...' : isVerifying ? 'Verifying email...' : isAdminLogin ? 'Admin Sign In' : 'Sign In'}
              </button>
            </form>
